...
int zjistiJas(int hodina) {
  ...
  if (hodina >= 12 && hodina < 18) return 5; // odpoledne
...
void setup() {
  ...
  // Nastavíme jas podle aktuální hodiny
  Time t = rtc.time();  // použij JEDNOU!
  int hodina = t.hr;
  strip.setBrightness(zjistiJas(hodina)); // použij funkci
  strip.show();
